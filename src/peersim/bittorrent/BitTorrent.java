...
public class BitTorrent implements EDProtocol {
    ...
    /**
     * The algorithm to be used to unchoke neighbours
     */
    private static final String PAR_UNCHOKING_ALGORITHM = "unchoking";
     ...
     * #BITFIELD} message is received and updated every time HAVE message is
     * received.
     */
    int rarestPieceSet[];
    /**
...
//                                                .getSortedInteractions(UPLOAD);
                                        Pair<Long, Integer> downloadInteractions = ((BitNode)
                                                node).getBestNode(DOWNLOAD);
                                        Pair<Long, Integer> uploadInteractions = ((BitNode)
                                                node).getBestNode(UPLOAD);
...
//                                        .getSortedInteractions(UPLOAD);
                    ...
                    } else System.out.println("Sender " + sender.getID() + " not" +
                            " alive");
                }
...
//                }
                ...
                while (!requestToServe.empty() && nPiecesUp < 10) {
                    ...
                    if (e != null && alive(req.sender)) {
                        ...
                        ((BitNode) node).addInteraction(req.time, req.sender.getID(), SENT,
                                UPLOAD, req.id);
                        cache[e.peer].justSent();
                        ...
                        // ((BitNode) req.sender).addInteraction(latency + downloadTime,
                        // node.getID(), 0, 0);
                    }
                }
            }
            ...
            case PIECE: // 9, PIECE message.
            {
                ...
                //TODO debug option - if set do the following
                try {
                ...
                belongs to the current downloading piece.*/
...
//                    }
                    ...
                    if (!sender.turnGoodInteraction(requestTime, node.getID(), UPLOAD, value)) {
                        System.out.println("Didn't find a upload interaction1");
                    }

                    //TODO Check if PIECE is a good one
                    ((BitNode) node).turnGoodInteraction(requestTime, sender.getID(), DOWNLOAD,
                            value);
                    ...
                    if (piece != currentPiece && status[piece] != 16) { //
                        ...
                        if (sender.isBadChunk()) {
                            ((BitNode) node).turnBadNOREPLYInteraction(requestTime, sender.getID
                                    (), DOWNLOAD, value);
                            sender.turnBadNOREPLYInteraction(requestTime, sender.getID(), UPLOAD,
                                    value);
                        } else {
                            ...
                            if (!sender.removeInteraction(node.getID(), SENT, UPLOAD, value)) {
                                //                                                    System.out
                                // .println("Remove duplicated " +"upload not succeed " +
                ...
                requested the block*/
                for (int i = 0; i < swarmSize; i++) {
                    if (alive(cache[i].node) && unchokedBy[i] &&
                            swarm[i][decode(block, 0)] == 1 && cache[i].node != sender) {
...
//                                    cache[i].node.getID()+";"+node.getID()+";"+value);
                ...
                if (status[currentPiece] == 16) { // if piece completed, I
                    ...
                    extract the queued received blocks */
                    ...
                    while (m > 0) { // I process the queue
                        ...
                        if (p == currentPiece && decode(pieceStatus[b], 0) != p) {
                            ...
                            temp.sender.turnGoodInteraction(temp.time, node.getID(), UPLOAD, temp
                                    .id);

                            //TODO Check if PIECE is a good one
                            ((BitNode) node).turnGoodInteraction(temp.time, temp.sender
                                    .getID(), DOWNLOAD, temp.id);
                            ...
                            {
                                incomingPieces.enqueue(temp.id, temp.sender, temp.time);
                                System.err.println("another piece");
                            } else // duplicate block
                            {
                                System.err.println("duplicate block!");
                                requestNextBlocks(node, pid, s.peer);
                                if (((BitNode) node).removeInteraction(temp.time, temp.sender
                                        .getID(), SENT, DOWNLOAD, temp.id)) {
                                    System.out.println("print  remove a interaction!");
                                } else {
                                    System.out.println("couldn't remove a download interaction "
                                            + "here " + node.getID() + " <- "
                                            + temp.sender.getID());
                                    if (node.getID() == 2 || temp.sender.getID() == 2) {
                                        System.out.println(node.getID() + " <- " + temp.sender
                                                .getID());
                                    }
                                }
                                if (temp.sender.removeInteraction(temp.time, node.getID(), SENT,
                                        UPLOAD, temp.id)) {
                                    System.out.println("print  remove a interaction!");
                                } else {
                                    System.out.println("couldn't remove a upload interaction " +
                                            "here" + temp.sender.getID() + " -> " + node.getID());
                                    if (node.getID() == 2 || temp.sender.getID() == 2) {
                                        System.out.println(temp.sender.getID() + " -> " + node
                                                .getID());
                                    }
                                }
                            }
            ...
            case PEERSET: // PEERSET message
            {
                ...
                for (int i = 0; i < peersetSize; i++) {
                    if (n[i] != null && alive(n[i].node) && search(n[i].node.getID()) == null &&
            ...
            case TRACKER: // TRACKER message
            {
                ...
                while (j < peersetSize) {
                    ...
                    for (int z = 0; z < j; z++) {
                        if (cache[i].node == null || tmp[z].node.getID() == cache[i].node.getID()
                ...
                interested than now, those in surplus will not be ordered. */
                    ...
                    case TRUST:
                        if (peerStatus == 1) {
                            ...
                            for (Pair<Long, Integer> neighInteractions : ((BitNode) node)
                                    .neighUploadInteractions.values()) {
                                Long nodeID = neighInteractions.getFirst();
                                Integer oldPoints = points.get(nodeID);
                                Integer newPoints = (oldPoints == null) ? 1 : oldPoints + 1;
                                points.put(nodeID, newPoints);
                            }

                            int nodesUnchoked = 4;
                            HashMap<Long, Integer> recomendedNodes = new HashMap<>(nodesUnchoked);
                            for (int i = 0; i < nodesUnchoked && points.size() > 0; i++) {
                                Map.Entry<Long, Integer> recomendedNode = points.entrySet()
                                        .stream().max(Comparator.comparing(Map.Entry::getValue))
                                        .get();
                                points.remove(recomendedNode.getKey());
                                recomendedNodes.put(recomendedNode.getKey(), recomendedNode
                                        .getValue());
                            }

                            Iterator<Long> iterator = recomendedNodes.keySet().iterator();
            ...
            case ANTISNUB_TIME: {
                ...
                for (int i = 0; i < nNodes; i++) {
                    if (byPeer[i].valueDOWN > 0 && (byPeer[i].valueDOWN - byPeer[i].head60) == 0)
                ...
                for (int i = 0; i < swarmSize; i++) {
					...
					/*If are at least 2 minutes (plus 1 sec of tolerance) that
					 ...
					 keepalive 2 minutes ago*/
                        ...
                        if (cache[i].lastSeen < (now - 121000) && cache[i].node != null &&
...
//                    .getSortedInteractions(UPLOAD);
            ...
            for (int j = 0; j < swarmSize; j++) {// send the interested
                ...
                if (alive(cache[j].node) && swarm[j][newPiece] == 1) {
                    long latency = ((Transport) node.getProtocol(tid)).getLatency(node, cache[j]
                            .node);
                    EDSimulator.add(latency, ev, cache[j].node, pid);
                    cache[j].justSent();
                }
    ...
    private int getPiece() {
        ...
        if (nPieceCompleted < 4) { //Uses random first piece
            ...
            for (int i = j + 1; i < nPieces; i++) { // Finds the rarest piece
                // not owned
                if (rarestPieceSet[i] < rarestPieceSet[rarestPieces[0]] && status[i] == 0) { //
                    // if strictly less than the
                    // current one
                    rarestPieces[0] = i;
                    nValues = 1;
                }
    ...
    private boolean greater(Element x, Element y) {
        ...
        if (peerStatus == 0) {
            if ((x.valueDOWN - x.head20) > (y.valueDOWN - y.head20)) return true;
            else return false;
        }
...
class Queue {
     ...
     * @param time   initial time of the interaction associated with this request
     * @return <tt>true</tt> if the request has been correctly added,
     * <tt>false</tt>
     * otherwise.
     */
    public boolean enqueue(int id, BitNode sender, long time) {
        if (dim < maxSize) {
            queue[tail % maxSize].id = id;
            queue[tail % maxSize].sender = sender;
            queue[tail % maxSize].time = time;
            tail++;
            dim++;
            return true;
        } else return false;
...
class Request {
    ...
    /**
     * The Initial time of the interaction associated with this request
     */
    public long time;
}